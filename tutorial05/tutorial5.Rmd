...
Adding additional predictors to a time-series linear model is a relatively simple procedure. The `tslm()` function in the `forecast` package will accept additional variables in a similar manner to the `lm()` function in base `R`. We will use the `BikeSharingDaily.csv` dataset to demonstrate the procedure. The first code chunk is take from the course reader, and is for preparing the data.
